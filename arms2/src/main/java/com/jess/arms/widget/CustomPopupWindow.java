...
public class CustomPopupWindow extends PopupWindow {
    ...
    private void initLayout() {
        mListener.initPopupView(mContentView, this);
    ...
    public interface CustomPopupWindowListener {
        public void initPopupView(View contentView, CustomPopupWindow customPopupWindow);
    }


    public void showDown() {
        if (mParentView == null) throw new IllegalStateException("mParentView is required");

        if (Build.VERSION.SDK_INT >= 24) {
            Rect visibleFrame = new Rect();
            mParentView.getGlobalVisibleRect(visibleFrame);
            int height = mParentView.getResources().getDisplayMetrics().heightPixels - visibleFrame.bottom;
            setHeight(height);
        }
